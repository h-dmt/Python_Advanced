# reate a function called grocery_store() that receives a different number of key-value pairs.
...
# If there are two or more products with the same quantity, sort them by their name's length
# in descending order. If there are two or more products with the same name's length,
...
def grocery_store(**products):
    ...
    for prod, qty in products.items():
        products_dct[prod] = qty
    products_dct = dict(sorted(products_dct.items(), key=lambda x: (-x[1], -len(x[0]), x[0])))
    return_string = [f"{p}: {products_dct[p]}" for p in products_dct]
    return '\n'.join(return_string)
